...
function BackDrops:set_images()
   -- Only get the list of images, don't load them into memory
   self.images = wezterm.glob(self.images_dir .. GLOB_PATTERN)
   self.images_cache = {}  -- Cache for loaded images
   self.cache_size = 5     -- Keep only 5 images in memory
   self.cache_order = {}   -- Track cache order for LRU eviction
   return self
end

---Load an image into cache if not already loaded
---Uses LRU (Least Recently Used) cache eviction
---@private
---@param idx number index of the image to load
---@return string? path to the image
function BackDrops:_load_image(idx)
   if not self.images[idx] then
      return nil
   end

   -- Check if already in cache
   if self.images_cache[idx] then
      -- Move to end of cache order (most recently used)
      for i, cached_idx in ipairs(self.cache_order) do
         if cached_idx == idx then
            table.remove(self.cache_order, i)
            break
         end
      end
      table.insert(self.cache_order, idx)
      return self.images_cache[idx]
   end

   -- Evict oldest image if cache is full
   if #self.cache_order >= self.cache_size then
      local oldest_idx = table.remove(self.cache_order, 1)
      self.images_cache[oldest_idx] = nil
   end

   -- Load the image path (not the actual image data)
   self.images_cache[idx] = self.images[idx]
   table.insert(self.cache_order, idx)
...
function BackDrops:_create_opts()
   ...
   -- Use lazy loading to get the image
   local image_path = self:_load_image(self.current_idx)
   ...
   return {
      {
         ...
         hsb = { brightness = 0.3 },  -- More efficient than opacity layering
      },
      {
         source = { Color = colors.background },
         height = '100%',  -- Reduced from 120% for better performance
         width = '100%',   -- Reduced from 120% for better performance
         opacity = 0.98,  -- Higher opacity = less blending work
      },
   }
end
...
---Create high-performance background options (minimal GPU usage)
---@private
---@return table
function BackDrops:_create_performance_opts()
   return {
      {
         source = { Color = self.focus_color },
         opacity = 1,
...
---Set the initial options for `background` with performance mode
---@param focus_on boolean? focus mode on or off
---@param performance_mode boolean? use minimal resource background
function BackDrops:initial_options(focus_on, performance_mode)
   focus_on = focus_on or false
   performance_mode = performance_mode or false
   assert(type(focus_on) == 'boolean', 'BackDrops:initial_options - Expected a boolean')
   assert(type(performance_mode) == 'boolean', 'BackDrops:initial_options - Expected a boolean for performance_mode')
   ...
   if performance_mode then
      return self:_create_performance_opts()
   end
